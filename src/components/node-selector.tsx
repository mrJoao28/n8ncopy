...
export function NodeSelector({
  ...
  const handleSelect = useCallback(
    (option: NodeTypeOption) => {
      ...
      const isTrigger = TRIGGER_OPTIONS.some(
        (trigger) => trigger.type === option.type,
      );
      if (isTrigger) {
        const alreadyExists = getNodes().some(
          (node) => node.type === option.type,
        );
        if (alreadyExists) {
      ...
      const centerScreenX = paneBounds
